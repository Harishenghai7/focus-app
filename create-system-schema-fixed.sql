-- CREATE SYSTEM DATABASE SCHEMA (FIXED)
...
DROP POLICY IF EXISTS "Boltz visible to auth users" ON boltz;
...
DROP POLICY IF EXISTS "Flash visible to auth users" ON flash;
DROP POLICY IF EXISTS "Users can insert own flash" ON flash;

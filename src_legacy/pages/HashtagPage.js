...
function HashtagPage({ user }) {
  ...
  const fetchHashtagPosts = async (page = 1) => {
    try {
      setLoading(true);
  ...
  const fetchTopPosts = async () => {
    try {
      ...
      // Fetch top posts using junction table
      const { data: postData, error } = await supabase
        .from('post_hashtags')
        .select(`
          posts (
      ...
      // Sort by like_count and take top 9
  ...
  const fetchRelatedHashtags = async () => {
    try {
      // Fetch hashtags that appear in similar posts
      const { data, error } = await supabase
        .from('hashtags')
        .select('*')
        .neq('tag', hashtag.toLowerCase())
        .order('post_count', { ascending: false })
        .limit(10);

      if (error) throw error;
      setRelatedHashtags(data || []);
    } catch (error) {
    ...
    >
          ...
          <div className="hashtag-info">
            ...
            <div className="hashtag-stats">
              <div className="hashtag-stat">
                ...
                <span>{formatNumber(hashtagData?.post_count || posts.length)} posts</span>
              </div>
            </div>
          </div>
      ...
      <div className="hashtag-content">
        ...
        {relatedHashtags.length > 0 && (
          <motion.div 
            className="related-hashtags-section"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
          >
            <h2 className="section-title">Related Hashtags</h2>
            <div className="related-hashtags-list">
              {relatedHashtags.map((tag, index) => (
                <motion.button
                  key={tag.id}
                  className="related-hashtag-chip"
                ...
                >
                  ...
                  <span className="hashtag-count">{formatNumber(tag.post_count || 0)}</span>
                </motion.button>
              ))}
            </div>
          </motion.div>
        ...
        <div ref={scrollRef} style={{ height: 1 }}></div>
      </div>
    </motion.div>
  );
}

...
export default function FollowRequests({ user }) {
  ...
  const confirmBulkApprove = () => {
    ...
    setConfirmDialog({
      isOpen: true,
      type: 'bulk-approve',
      data: { count: selectedRequests.size }
    });
  };
  ...
  return (
    <Layout user={user}>
                    ...
                    >
                      ...
                      <div className="request-info">
                        ...
                        <div className="request-time">
                          {formatDate(request.created_at, { month: 'short', day: 'numeric' })}
                        </div>
                      </div>
                    </div>
      ...
      <ConfirmDialog
        ...
        message={
          confirmDialog.type === 'bulk-approve'
            ? `Are you sure you want to approve ${confirmDialog.data?.count} follow request(s)?`
            : `Are you sure you want to reject ${confirmDialog.data?.count} follow request(s)?`
        }
        confirmText={confirmDialog.type === 'bulk-approve' ? 'Approve' : 'Reject'}
        cancelText="Cancel"
        onConfirm={handleConfirmDialogConfirm}
        onCancel={handleConfirmDialogCancel}
        variant={confirmDialog.type === 'bulk-reject' ? 'danger' : 'primary'}
      />
    </motion.div>
    </Layout>
  );

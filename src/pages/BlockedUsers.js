...
export default function BlockedUsers({ user }) {
  ...
  const [confirmDialog, setConfirmDialog] = useState({ open: false, blockId: null, blockedUserId: null });
  ...
  const openConfirmDialog = (blockId, blockedUserId) => {
    setConfirmDialog({ open: true, blockId, blockedUserId });
  };

  const closeConfirmDialog = () => {
    setConfirmDialog({ open: false, blockId: null, blockedUserId: null });
  };

  const handleUnblock = async () => {
    const { blockId, blockedUserId } = confirmDialog;
  ...
  const filteredBlockedUsers = (blockedUsers || []).filter((block) => {
    ...
    return (
      block.blocked?.username?.toLowerCase().includes(query) ||
      block.blocked?.full_name?.toLowerCase().includes(query)
    );
  });
    ...
    <Layout>
      ...
      >
                  ...
                  >
                    <div className="blocked-user-info">
                      <img
                        ...
                        alt={block.blocked?.username || "User"}
                    ...
                    <button
                      ...
                      onClick={() => openConfirmDialog(block.id, block.blocked_id)}
        ...
        <ConfirmDialog
          ...
          title="Unblock User"
          message="Are you sure you want to unblock this user? They will be able to see your posts and interact with you again."

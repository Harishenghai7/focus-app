...
class CallSignalingService {
  ...
  /**
   * Send ICE candidate (with batching support)
   * @param {string} callId - Call UUID
   * @param {RTCIceCandidateInit|RTCIceCandidateInit[]} candidates - ICE candidate(s)
   */
  async sendIceCandidate(callId, candidates) {
    try {
      // Handle both single candidate and batch of candidates
      const candidateArray = Array.isArray(candidates) ? candidates : [candidates];
      
      const signalPromises = candidateArray.map(candidate => 
        supabase
          .from('call_signaling')
          .insert({
            call_id: callId,
      ...
      // Send all candidates in parallel
      await Promise.allSettled(signalPromises);
      console.log(`📡 Sent ${candidateArray.length} ICE candidate(s)`);
    } catch (error) {
      console.warn('⚠️ Failed to send ICE candidates:', error);

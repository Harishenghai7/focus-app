...
/**
 * UserOptionsMenu - Menu for user actions (block, mute, report).
 * @component
 * @param {Object} targetUser - Target user object
 * @param {Object} currentUser - Current user object
 * @param {function} onClose - Handler to close menu
 * @param {function} onAction - Handler for action
 * @returns {React.ReactElement}
 */
const UserOptionsMenu = React.memo(function UserOptionsMenu({ targetUser, currentUser, onClose, onAction }) {
  ...
  return (
    ...
    >
      <motion.div 
        className={styles.userOptionsMenu}
      ...
      >
        <button 
          className={`${styles.optionBtn} ${styles.danger}`}
          onClick={handleBlock}
          disabled={loading}
          aria-label={`Block ${targetUser.username}`}
          role="menuitem"
        >
        ...
        <button 
          ...
          onClick={handleMute}
          disabled={loading}
          aria-label={`Mute ${targetUser.username}`}
          role="menuitem"
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        <button 
          ...
          aria-label={`Report ${targetUser.username}`}
          role="menuitem"
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        <button 
          className={`${styles.optionBtn} ${styles.cancel}`}
          onClick={onClose}
          aria-label="Cancel"
          role="menuitem"
        >
...
UserOptionsMenu.displayName = 'UserOptionsMenu';

...
/**
 * MediaSelector - Allows users to select, validate, and compress media files.
 * @component
 * @param {Array} selectedMedia - Array of selected media files
 * @param {function} onMediaChange - Handler for media change
 * @param {number} maxItems - Maximum number of items allowed
 * @returns {React.ReactElement}
 */
const MediaSelector = React.memo(function MediaSelector({ 
  selectedMedia = [], 
  onMediaChange, 
  maxItems = 10 
}) {
  ...
  return (
    <div className={styles.mediaSelector}>
      {/* File Input Area */}
      <div
        className={`${styles.mediaSelectorDropzone} ${dragOver ? styles.dragOver : ''} ${selectedMedia.length >= maxItems ? styles.disabled : ''}`}
      ...
      >
        ...
        <div className={styles.dropzoneContent}>
          ...
          <p className={styles.dropzoneCounter}>
      ...
      {compressionProgress && (
        <motion.div 
          className={styles.mediaSelectorProgress}
        ...
        >
          ...
          <div className={styles.progressFilename}>{compressionProgress.currentFile}</div>
          ...
          <div className={styles.progressPercentage}>{compressionProgress.progress || 0}%</div>
        </motion.div>
      )}
      ...
      {error && (
        <motion.div 
          className={styles.mediaSelectorError}
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <pre>{error}</pre>
          <button onClick={() => setError('')} aria-label="Close error message">
            ×
          </button>
        </motion.div>
      )}
            ...
            {selectedMedia.map((media, index) => (
              ...
              >
                ...
                <div className={styles.mediaPreview}>
                  {media.type === 'video' ? (
                    <video src={media.url} className={styles.mediaThumbnail} aria-label={`Video: ${media.file.name}`} />
                  ) : (
                    <img src={media.url} alt={`Selected ${index + 1}`} className={styles.mediaThumbnail} aria-label={`Image: ${media.file.name}`} />
                  )}
                  ...
                  {/* Position Number */}
                  <div className={styles.mediaPosition}>{index + 1}</div>
                ...
                <div className={styles.mediaInfo}>
                  <p className={styles.mediaFilename}>{media.file.name}</p>
                  <p className={styles.mediaFilesize}>
                    {(media.file.size / 1024 / 1024).toFixed(2)} MB
                    {media.compressionStats && (
                      <span className={styles.compressionBadge} title={`Saved ${media.compressionStats.savedPercentage}%`}>
      ...
      {/* Helper Text */}
      {selectedMedia.length > 0 && (
        <p className={styles.mediaSelectorHint}>

...
export default function RealtimeNotifications({ user, onUnreadCountChange }) {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', {
          ...
          if (data) {
            ...
            if (document.hidden) showPushNotification(data);
          }
        })
        .on('postgres_changes', {
          event: 'UPDATE',
  ...
  return (
    <>
      {/* WebSocket connection indicator for QA testing */}
      <div 
        className="realtime-connection-indicator" 
        data-testid="websocket-connection"
        data-connected={!!subscriptionRef.current}
        style={{ display: 'none' }}
        aria-hidden="true"
      />

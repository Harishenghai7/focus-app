...
const FocuslyLion = ({
    ...
    const getMouthPath = () => {
        ...
        switch (emotion) {
            ...
            case 'idle_nervous':
                return "M42,67 Q46,69 50,67 Q54,65 58,67"; // Nervous pressed lips
            case 'guardian_mode':
                return "M40,67 Q50,70 60,67"; // Firm confident smile
            case 'sleepy':
    ...
    // Map emotion to CSS class for body-part animations
    const getEmotionClass = () => {
        const validClasses = ['idle', 'idle_nervous', 'guardian_mode', 'thinking_focus',
            'happy_celebrate', 'concerned', 'happy', 'sad', 'speaking'];
        const base = isSpeaking ? 'speaking' : (emotion || 'idle');
        return validClasses.includes(base) ? styles[base] : styles.idle;
    };

    return (
        <svg
            viewBox="0 0 200 220"
            className={`${styles.lionSvg} ${getEmotionClass()} ${className}`}
